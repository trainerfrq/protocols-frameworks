...
public class SwaggerDocumentationConfig {
    ...
    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("UNIT Converter API").description("").license("proprietary").licenseUrl("http://www.apache.org/licenses/LICENSE-2.0.html")
    ...
    public Docket customImplementation() {
        return new Docket(DocumentationType.SWAGGER_2).select().apis(RequestHandlerSelectors.basePackage("com" + "" +
                ".example.training.converterapi.controller")).build().apiInfo(apiInfo());
    }
